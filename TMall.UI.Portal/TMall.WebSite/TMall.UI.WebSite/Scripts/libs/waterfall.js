    ...
    Waterfall.prototype = {
        ...
        */
        _getScrollTop: function () {
            ...
            scrollPos = scrollPos <= this.$element.offset().top ? 0 : scrollPos - this.$element.offset().top;

            this._debug('scrollTop:', scrollPos);
            this._debug('$element.offset().top:', this.$element.offset().top);
        ...
        * _scroll
        */
        _scroll: function (scrollSign) {
            var options = this.options,
                state = options.state,
                self = this;

            this._pinManage(scrollSign);
        ...
        */
        _doScroll: function () {
            ...
            $window.bind('scroll', function () {
                ...
                scrollTimer = setTimeout(function () {
                    ...
                    self.scrollSign=new Date().getTime();
                    self._scroll(self.scrollSign);
                },5);
            });
        },
         ...
         *管理可视区域的砖块，提供性能
         */
        _pinManage:function(scrollSign){
            
            if(this.scrollSign!=scrollSign)
                return;

            var classthis=this;
            var scrolltop= this._getScrollTop();

            var bufferDis=350;//缓冲距离

            var topDis=scrolltop-bufferDis;
            var downDis=$window.height()+scrolltop+bufferDis;

            var $pin = this.$element.find('.' + this.options.itemCls);

            var addPin=this._getPinInCache(topDis,downDis);

            $pin.each(function(i){
              
              if(classthis.scrollSign!=scrollSign)
                  return;

              var $this= $(this);
              var position=$this.position();
              if(position.top +$this.height()<topDis||
                 position.top> downDis)
              {
                classthis._addPinToCache($this);
                $this.remove();
              }

              for(var i=0,h=addPin.length;i<h;i++)
              {
               if(classthis.scrollSign!=scrollSign)
                  return;

                if(addPin[i]!=undefined && addPin[i].top==position.top && addPin[i].left==position.left){
                    addPin[i]=undefined;
                    break;
                }
              }
             ...
             for(var i=0,h=addPin.length;i<h;i++)
              {
               ...
               if(classthis.scrollSign!=scrollSign)
                ...
                if(addPin[i]!=undefined){
                    addPin[i].value.appendTo(this.$element);
                }
              }
        },

        _addPinToCache:function(pin){
            if(this._existPinInCache(pin))
                return;
            ...
            this.pinCache.push(item);
            this._debug('pinCache length', this.pinCache.length);
        },

        _existPinInCache:function(pin){
            
            if(this.pinCache.length==0) return false;

            var item;
            for(var i=0,h=this.pinCache.length;i<h;i++)
            {
                item=this.pinCache[i];
                if(item.top==pin.position().top && item.left==pin.position().left)
                    return true;
            }
            return false;
        },

        _getPinInCache:function(top,down){
            var data=[];
            var item;
            for(var i=0,h=this.pinCache.length;i<h;i++)
            {
                item=this.pinCache[i];
                if(item.top>=top && item.top+item.value.height()<=down)
                    data.push(item);
            }
